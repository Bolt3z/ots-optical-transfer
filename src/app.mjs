...
const RX = {
  ...
  reset() {
    ...
    this.startToken = (this.startToken || 0) + 1;   // invalida un avvio in corso
    this.st = {
  ...
  async startCamera() {
    ...
    // getUserMedia prima dei worker: e' la chiamata che dipende dal gesto
    // dell'utente, e Safari non aspetta.
    try {
    ...
    const v = $("rx-video");
    v.srcObject = this.stream; v.hidden = false; v.controls = false;
    const mine = this.startToken;
    try { await v.play(); }
    catch (err) { this.fail("Il video non parte: " + err.name); return; }
    await this.startScanner();
    if (mine !== this.startToken) return;
    this.running = true;
    this.scanLoop(v);
  },
   ...
   * avvia dopo, e il video si riavvolge quando e' pronto.
   */
  startVideoFile(file) {
    this.reset();
    ...
    v.controls = true;              // un play nativo, se il nostro non basta
  ...
  async videoStarted(v) {
    const mine = this.startToken;
    v.pause();                      // si aspetta lo scanner, senza perdere l'inizio
    await this.startScanner();
    ...
    v.currentTime = 0;
    try { await v.play(); }
    catch (err) { this.fail("Il video si è fermato: " + err.name); }
  },
